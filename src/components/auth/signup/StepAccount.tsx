  ...
  return (
    <div className="bg-white rounded-2xl border-2 border-slate-200 p-6 shadow-xl relative">
      ...
      <div className="mb-6">
        ...
        <p className="text-slate-600 text-sm">입력하신 정보로 회원님의 계정이 생성됩니다.</p>
      </div>

      <div className="space-y-4 mb-6">
        <EmailVerification value={email} onChange={setEmail} onVerified={setEmailVerified} verified={emailVerified} />
        <PasswordField value={password} onChange={setPassword} onValidated={setPasswordValidated} confirmValue={password2} onConfirmChange={setPassword2} />
      </div>
      ...
      <div className="flex gap-3">
        ...
        <button onClick={onNext} disabled={!isAccountValid} className="flex-1 next-button text-white py-3 rounded-md font-bold text-sm transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">다음</button>
      </div>
    </div>
  );
};
