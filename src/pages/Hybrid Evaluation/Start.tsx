...
const StartHybridEvaluation = () => {
  ...
  const handleCashflowUpdate = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        throw new Error(
          response.message || '현금흐름 데이터 업데이트에 실행했습니다.',
        );
      }
    } catch (error: any) {
  ...
  // 신용평가 데이터 업데이트 함수
  const handleCreditDataUpdate = async () => {
    if (!sessionId) {
      console.error('세션 ID가 없습니다.');
      setError('세션 ID가 없어 신용평가 데이터 업데이트를 진행할 수 없습니다.');
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      // sessionId 검증
      console.log(
        '🚀 [StartHybridEvaluation] handleSubmit - using sessionId:',
        sessionId,
      );
      if (!sessionId) {
        throw new Error(
    ...
    <>
      ...
      >
            ...
            <SalesSection
              ...
              onCsvUploadComplete={(fileName) => {
                ...
                try {
                  localStorage.setItem(
                    'hybridStart.completed',
                    JSON.stringify({ ...completed, sales: true }),
                  );
                  localStorage.setItem(
                    'hybridStart.files',
                    JSON.stringify({ ...fileNames, sales: fileName }),
                  );
                } catch (storageError) {
                  console.warn('로컬 스토리지 저장 실패:', storageError);
                }
                  ...
                  <tbody>
                    <tr className="odd:bg-white even:bg-gray-50/60">
                      ...
                      <td className="border border-gray-200 p-2" rowSpan={6}>
                        단기연체의 기준은 5영업일 10만원 이상이며, 장기연체는
                        90일 이상 연체 등을 기준으로 하고 있습니다. 단, 임시적
        ...
        >
          <div className="space-y-3 text-sm text-gray-700">
            <p>
              현금흐름 분석을 위해 사업자(또는 대표자) 명의의 은행 계좌를
              연결하고 최근 거래내역 조회 동의해 주세요.
            </p>
            <div className="rounded-md border border-gray-200 p-3">
