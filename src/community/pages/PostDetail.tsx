        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
            <div className="w-80 rounded-2xl bg-white p-5 shadow-xl">
                ...
                <div className="mt-5 flex justify-end gap-2">
                    <button className="rounded-lg bg-gray-200 px-3 py-2 hover:bg-gray-300" onClick={onClose}>
                        {cancelText}
                    </button>
                    <button
...
export default function PostDetailPage() {
    ...
    const onBookmarkToggle = async () => {
        ...
        try {
            ...
            const next = (r.data?.data as ToggleBookmarkRes | undefined)?.bookmarked ?? !bookmarked;
    ...
    return (
        <div className="space-y-4 pb-4">
              ...
              #{h.name}
                ...
                {images.length > 0 && (
                    <div className="mt-3 grid grid-cols-2 gap-2">
                        {images.map((src, i) => (
                            <img key={i} src={src} className="h-36 w-full rounded-xl border object-cover" />
                        ))}
                    </div>
                )}

                {/* 본문 */}
                <div className="mt-4 whitespace-pre-wrap text-[15px] leading-6">{data.content}</div>
          ...
          <span className={counterCls} title="조회수">
            ...
            {/* 삭제 확인 모달 */}
            <ConfirmModal open={deleteOpen} onClose={() => setDeleteOpen(false)} onConfirm={onDelete} />
        </div>
    );
}
        ...
        <div className="rounded-xl border p-3">
            ...
            <div className="mb-1 flex items-center gap-2 text-sm">
                <div className="flex items-center gap-2">
                    <div className="h-6 w-6 rounded-full bg-gray-200" aria-hidden /> {/* 아바타 자리 */}
              ...
              작성자
                ...
                <span className="ml-1 text-xs text-gray-500">· {fromNow(c.createdAt)}</span>
            </div>

            {/* 본문 / 편집 */}
            {editing ? (
                <div className="space-y-2">
                    <textarea className="textarea" value={val} onChange={(e) => setVal(e.target.value)} />
                ...
                <div className="whitespace-pre-wrap text-[15px] leading-6">{c.content}</div>
            )}
