...
export default function PostCreate() {
  ...
  const allowedEtc: HashtagType[] =
    category === 'CASE' ? ['GENERIC'] : ['POST_TYPE', 'GENERIC'];
        ...
        <Card className="p-4 space-y-4">
          ...
          {category === 'FREE' && (
            <div className="flex gap-2">
              {(['QUESTION', 'PROMO', 'TIP', 'OTHER'] as FreeType[]).map(
                (t) => (
                  <Button
                    key={t}
          ...
          {/* 승인 사례일 때 업종 칩 노출 */}
          {category === 'CASE' && (
            <SectorSelector selected={sectorTags} onChange={setSectorTags} />
          )}
