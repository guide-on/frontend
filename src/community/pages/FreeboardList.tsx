...
export default function FreeboardList(){
    ...
    const loadMore=async ()=>{
        ...
        setError(undefined);
        try{
            const res=await api.get("/community/posts",{params:{category:"FREE", freeType, page, size, sort}});
    ...
    const {sentinelRef}=useInfiniteScroll(loadMore,hasMore,[sort,freeType,page,items.length,total]);
        ...
        <div className="space-y-3">
            ...
            <div className="card p-3 flex flex-wrap gap-2">
                ...
                {(['QUESTION','PROMO','TIP','OTHER'] as FreeType[]).map(t=>(
                    <button key={t} className={`chip ${freeType===t?'chip-blue':'chip-gray'}`} onClick={()=>setFreeType(freeType===t?undefined:t)}>
                        {t==='QUESTION'?'질문':t==='PROMO'?'홍보':t==='TIP'?'마케팅팁':'기타'}
                    </button>
                ))}
            </div>
