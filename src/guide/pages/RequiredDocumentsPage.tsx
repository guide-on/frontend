...
export function RequiredDocumentsPage() {
  ...
  const [totalRequirements, setTotalRequirements] = useState<number | null>(
    null,
  );
  ...
  const loadDocuments = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        ...
        statusData.success !== false ? 'SUCCESS' : 'FAILED',
      );
      ...
      const documentGroups = statusData.documentGroups.map((group) => ({
        ...
        documents: group.documents.map((doc) => ({
          ...
          status:
            doc.uploadStatus === 'COMPLETED' || doc.uploadStatus === 'UPLOADED'
              ? ('completed' as const)
              : ('pending' as const),
        })),
      ...
      if (typeof statusData.progressPercentage === 'number')
        setProgressPercentage(Math.round(statusData.progressPercentage));
      if (typeof statusData.totalRequirements === 'number')
        setTotalRequirements(statusData.totalRequirements);
      if (typeof statusData.completedRequirements === 'number')
        setCompletedRequirements(statusData.completedRequirements);

      // 최초 로드 시 마이데이터 연동 체크 및 리다이렉트
      console.log('🔍 마이데이터 체크:', {
        hasMydataChecked,
        documentGroupsLength: documentGroups.length,
        sessionId,
      });
        ...
        if (hasMydataDocuments) {
          ...
          const hasSyncedDocuments = documentGroups.some((group) =>
            group.documents.some(
              (doc) => doc.mydataEligible && doc.status === 'completed',
            ),
          );
  ...
  const getGroupStatus = (
    group: DocumentGroup & { completedCount?: number; isCompleted?: boolean },
  ) => {
    const completedCount =
      typeof group.completedCount === 'number'
        ? group.completedCount
        : group.documents.filter((doc) => doc.status === 'completed').length;
  ...
  const onBack = () => nav(-1);
  const onGroupClick = (group: DocumentGroup) =>
    nav(`/guide/upload/${sessionId}/${group.groupKey}`);
